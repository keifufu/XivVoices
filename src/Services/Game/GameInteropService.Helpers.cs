...
public partial class GameInteropService
{
  ...
  public string GetRaceFromSkeleton(int id, ushort region)
  {
    if (_dataService.Manifest != null)
    {
      if (_dataService.Manifest.RaceMappings.TryGetValue((id, region), out string? name))
        return name;
      else if (_dataService.Manifest.RaceMappings.TryGetValue((id, 0), out string? defaultName))
        return defaultName;
    }
